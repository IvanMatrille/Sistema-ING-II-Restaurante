...
<div class="container" style="margin:20px;border: solid black 1px;  border-radius:10px; margin:auto; float:left; margin-right:50px">
    <form class="form-" action="registrarPedido.php" method="get" onsubmit="">
        <div class="form-group">
            <label for="" style="font-size:15px">num pedido:</label>
            <input style="float:left" readonly  type="number" class="form-control" id="idPedido" name="numeroDelPedido">
        </div>
        <div class="form-group">
            <label for="" style="font-size:15px">mesero:</label>
            <input style="float:left" readonly type="number" class="form-control" id="idMesero" name="idMesero">
        </div>
        <div class="form-group">
            <label for="" style="font-size:15px">fecha:</label>
            <input id="fecha" readonly style="float:left" class="form-control" id="idFecha" name="fecha">
        </div>
        <br>
        <label for="email">Mesa numero:</label>
        <select id="mesasExistentes" class="form-control" name="idMesa">
        <?php 
        ...
        <label>Menu:</label>
        <div>
            <select style="" class="form-control vacia" id="listaProducto" class="selectpicker" name="producto">
        <?php 
            $valor = 0;
            $idProducto = [];
            $totalID =1;
            
            $query = "SELECT DISTINCT id,nombre,precio FROM producto";
            $sql = mysqli_query($Conexion,$query);
            ...
            $query = "SELECT MAX(id) as MAX FROM pedido";
            $sql = mysqli_query($Conexion,$query);
        ...
        <br>
        <input id="btnAgregar" type="button" class="add-row btn btn-primary" value="Agregar">
        <button id="btnEnviar" type="submit" class="btn btn-success">Enviar Pedido</button>
        <br><br>
        <div class="container" style="width: 100%; border: solid black 1px;  border-radius:10px; margin:auto; float:left;">
            <table id="table" class="table">
                ...
                <input readonly hidden name ='articulos' id="totalArticulos"/>
            </table>
            <div class="form-group">
                <button type="button" class="delete-row btn btn-primary">Borrar Item</button>
            </div>
        </div>
    <div>
</form>
